...
class CommunitiesDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Future<void> loadAllData(BuildContext context) async {
      ...
      await Future.wait([
        context.read<CommunitiesBloc>().stream.firstWhere((state) => !state.isSingleCommunities),
      ]);
    }
    return Scaffold(
      appBar: AppBar(),
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting ||  communitiesState.isSingleCommunities == true ) {
                    ...
                    children: [
                      if (data.instagramLink.isNotEmpty)
                        IconButton(
                          ...
                          onPressed: () => launchUrl(Uri.parse(data.instagramLink)),
                        ),
                      ...
                      if (data.youtubeLink.isNotEmpty)
                        IconButton(
                          ...
                          onPressed: () => launchUrl(Uri.parse(data.youtubeLink)),
                        ),
                    ],
                  ),
